...
class CLIP_Base:
    ...
    def encode_image(self, img, input_range=None, apply_preproce=True):
        if apply_preprocess:
            imgs = self.preprocess(imgs, input_range=None)
        img_embeddings = self.module.encode_image(imgs)
    ...
    def encode_texts(self, encode_texts):
        text_embeddings = torch.stack(
            [
                self.model.encode_text(clip.tokenize(text).to(self.device))
                .detach()
                .clone()
                for text in texts
            ]
