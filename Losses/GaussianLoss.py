...
def gkern(ylen=256, xlen=256, stdy=128, stdx=128):
    # return 2d gaussian kernal array
    gkerny = gaussian_fn(ylen, std=stdy)
    gkernx = gaussian_fn(xlen, std=std)
    gkern2d = torch.outer(gkerny, gkernx)

    return gkern2d


def GaussianLoss(LossInterface):
    ...
    def get_loss(self, cur_coutouts, outs, args, globals=None, lossGlobals=None):
