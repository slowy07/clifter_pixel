...
from Losess.LossInterface import LossInterface


class GaussianSmoothing(nn.module):
    def __init__(self, channels, kernel_size, sigma, dim=2):
        super(GaussianSmoothing, self).__init__()
        if isinstance(kernel_size, number.Number):
        ...
        if dim == 1:
            ...
            raise RuntimeError(
                "only 1, 2 and 3 dimension are not supported. received {}".format(dim)
            )
...
class SmoothnessLoss(LossInterface):
    ...
    def get_loss(self, cur_cutouts, out, args, globals=None, lossGlobals=None):
        cur_loss = []
        for _, cutouts in cur_cutouts.item():
